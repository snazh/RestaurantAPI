...
async def post_order(request: schemas.OrderSchema, db: Session = Depends(database.get_db)):  # create order
...
async def get_specific_order(order_id: int, db: Session = Depends(database.get_db)):
...
async def update_order(order_id: int, request: schemas.OrderSchema, db: Session = Depends(database.get_db)):
